...
void D3DMenu::Show(DxManager *pDxm)
{
	int i, val, cy;
	DWORD color;
	...
	cy = y;
	if (title) {
		pDxm->DrawString((float)(x + totwidth / 4), (float)cy - 3, col_title, title);
		cy += titleheight;
	}
	for (i = 0; i < noitems; i++) {
		if (MENU[i]->typ == MENUTEXT)
		{
			pDxm->DrawString((float)x, (float)cy, col_on, MENU[i]->txt);
			if (MENU[i]->opt)
			{
				pDxm->DrawString((float)(x + ofs), (float)cy, col_on, (char *)MENU[i]->opt);
			}
		}
		else
		{
			...
			if (i == cur)
				...
				color = (val) ? col_on : col_off;
			pDxm->DrawString((float)x, (float)cy, color, MENU[i]->txt);
			if (MENU[i]->opt)
			{
				pDxm->DrawString((float)(x + ofs), (float)cy, color,  (char *)MENU[i]->opt[val]);
			}
		}
		cy += height;
	}
